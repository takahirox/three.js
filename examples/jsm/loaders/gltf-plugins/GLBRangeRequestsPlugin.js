...
export default class GLBRangeRequestsPlugin {
	...
	static load( url, loader, onLoad, onProgress, onError, fileLoader = null ) {
		...
		GLBRangeRequestsPlugin.loadContent( url, fileLoader )
			.then( content => {
				...
				loader
					.register( parser => new GLBRangeRequestsPlugin(
						parser,
						url,
	...
	static async loadContent( url, fileLoader = null ) {
		...
		if ( header.version < 2.0 ) {
			...
			return Promise.reject( new Error( 'GLBRangeRequestsPlugin: Legacy binary file detected.' ) );
		...
		if ( result.binChunkOffset === null ) {
			...
			return Promise.reject( new Error( 'GLBRangeRequestsPlugin: No BIN chunk so progressive glb loading is not needed.' ) );
		...
		if ( result.jsonContent.extensionsUsed &&
			...
			return Promise.reject( new Error( 'GLBRangeRequestsPlugin: currently no EXT_meshopt_compression extension support.' ) );
	...
	loadBufferView( bufferViewIndex ) {
		...
		return loadPartially( this.parser.fileLoader, this.url, this.binChunkOffset + offset, length );
